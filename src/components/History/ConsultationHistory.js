...
import { TicketTransactionData, UserData } from '../Data/Data';
      ...
      {TicketTransactionData && 
        ...
        <tr className={`w-full font-semibold ${menuTransactionOpen[index] ? '' : 'hidden'}`}>
          ...
          <td rowSpan={2} className="pt-4 mx-2 pb-4">
            <button className={`p-2 rounded-lg bg-red-500 text-white font-semibold ${user.authority_name === 'depositaire' ? 'hidden' : ''} ${user.authority_name === 'caissier' ? 'hidden' : ''}  ${!ticketTransaction.sold ? 'hidden' : ''}`} onClick={() => handleActivatedClick(ticketTransaction)}>Annuler</button>
          </td>
        </tr>
